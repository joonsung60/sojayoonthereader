...
def _paginate(lines: list[str], max_lines: int, overlap: int = 3) -> list[list[str]]:
    """한 화면 행 수 초과 시 두 페이지로 분할 (overlap행 겹침)."""
    if len(lines) <= max_lines:
        return [lines]
    page1 = lines[:max_lines]
    page2 = lines[max_lines - overlap:]
    return [page1, page2]
...
# ── 영상 생성 ────────────────────────────────────────────
def generate_video(test: bool = False, test_short: bool = False):
    ...
    body_top, body_bottom = 280, VIDEO_H - 60 - 40
    ...
    print(f"🎬 영상 생성 시작 — 배경: {bg_path.name}, 음원: {audio_path.name}"
          + ("  [test-short: 첫 3편]" if test_short else ""))
...
if __name__ == "__main__":
    ...
    generate_video(test=args.test, test_short=args.test_short)
